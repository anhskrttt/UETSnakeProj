...
void Game::handleEvent() {
	while (SDL_PollEvent(&gEvent) != 0) {
		...
		//change background according to status (start(0), playing(1), end(2))
		if (status_ == 1) {
			player->handleInputAction(gEvent); //playing
		}
		else if (status_ == 0) {
			if (gEvent.type == SDL_KEYDOWN) status_++; //starting background
		}
		else {
			//ending background
...
void Game::update() {
	...
	//update game object while playing
	if (status_ == 1) {
		if (player->isDead()) {
			Mix_HaltMusic();
			status_ = 2;
		...
		//update ending-background's music
		if (!Mix_PlayingMusic()) {
...
//Display game object on the screen
void Game::render() {
